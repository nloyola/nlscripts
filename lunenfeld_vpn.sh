...
resolv_conf_dns_broken() {
  # The failure this catches: the VPN client replaces /etc/resolv.conf with a flat
...
reassert_tailscale() {
  ...
  if [[ "$op" == "down" ]]; then
    ...
    echo "lunenfeld_vpn.sh: no resolver in /etc/resolv.conf works; restarting tailscaled." >&2
    sudo systemctl restart tailscaled 2>/dev/null || true
  fi
}
...
require_forticlient() {
  ...
  echo "lunenfeld_vpn.sh: /opt/forticlient/fortivpn is not installed." >&2
  echo "lunenfeld_vpn.sh: FortiClient Linux is a manual, registration-gated download -" >&2
  echo "lunenfeld_vpn.sh: https://www.fortinet.com/support/product-downloads" >&2
  return 1
}
